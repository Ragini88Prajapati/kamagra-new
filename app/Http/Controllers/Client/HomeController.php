...
class HomeController extends Controller
{
    ...
    {
        $data['product_list'] = Product::where('status', 1)->whereHas('productVariant')->select('name', 'slug', 'id', 'mrp', 'price', 'subtitle', 'discount', 'image', 'image_path', 'gender_id')->orderByDesc('id')->limit(6)->get();
    ...
    public function test_email()
    {
        ...
        $data['order_data'] = Order::where('users_id', $user_id)
            ->where('id', 7)
            ->with([
                'order_product.product.brand',
                'order_product.delivery_status',
                'user_detail',
                'user',
                'state'
            ])
